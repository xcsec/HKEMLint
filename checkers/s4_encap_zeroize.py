...
class S4EncapZeroizeChecker(BaseChecker):
    ...
    def _check_s4_2(self, cpg: FunctionCPG, cfg: nx.DiGraph,
        ...
        if not combiners:
            return self._check_s4_2_fallback(cpg, cfg, comp1, comp2, project)

        first_op = min(comp1 + comp2, key=lambda n: n.line)
        second_op = max(comp1 + comp2, key=lambda n: n.line)
        all_op_ids = {n.id for n in comp1 + comp2}
    ...
    def _check_s4_2_fallback(self, cpg: FunctionCPG, cfg: nx.DiGraph,
                              comp1: list[CPGNode], comp2: list[CPGNode],
                              project: str) -> list[Finding]:
        findings: list[Finding] = []
        first_op = min(comp1 + comp2, key=lambda n: n.line)
        ...
        if between or after:
            findings.append(Finding(
                ...
                evidence=(
                    ...
                    f"{'between' if between else 'after'} encap ops."
                ),
            ...
            findings.append(Finding(
                ...
                verdict=Verdict.FAIL, severity=Severity.HIGH,
                evidence=(
                    f"No COMBINER and no zeroize between ENCAP_1 "
