"""S2 — Combiner Security.

S2-1  Weak Combiner
      Graph query: Find COMBINER nodes. Classify as KDF / XOR / concat.
      XOR or raw concat without downstream KDF → FAIL.

S2-2  Missing Context Binding
      Graph query: Find COMBINER(val=K). Check dataflow predecessors
      for c_1, c_2, ek_1, ek_2 (context elements).
      Missing → FAIL.
"""
...
def _classify_combiner(node: CPGNode) -> str:
    """Return 'kdf', 'xor', 'concat', or 'unknown'."""
...
class S2CombinerChecker(BaseChecker):
    """S2-1 Weak Combiner + S2-2 Missing Context Binding."""
    ...
    def check(self, cpg: FunctionCPG) -> list[Finding]:
        ...
        # Filter to combiners that receive shared secrets
        ...
        # S2-2: context binding (once for all combiners)
        if ss_combiners:
            findings.extend(self._check_s2_2(cpg, ss_combiners, project))

        # Fallback XOR detection if no XOR FAIL found yet
    ...
    def _check_s2_1(self, cpg: FunctionCPG, cfg: nx.DiGraph,
        ...
        if ctype == "concat":
            # Is a KDF reachable downstream via CFG?
    ...
    def _check_s2_2(self, cpg: FunctionCPG,
                     ...
                     project: str) -> list[Finding]:
        """Check if combiner binds ct + pk + label + algID."""
        ...
        # Fallback: text-window when dataflow sparse
        non_none = sum(
