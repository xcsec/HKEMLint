"""S5 — Undestroyed Intermediate Key Material (Decapsulation).

S5-1  Undestroyed Intermediate on Half-Success (Decaps)
      Graph query: Find DECAP(comp=1) producing K_1, then DECAP(comp=2).
      On error path from DECAP_2 to EXIT → check if ZEROIZE(K_1) exists.

S5-2  Undestroyed Intermediate on Completion (Decaps)
      Graph query: After both DECAP nodes and COMBINER(K), trace CFG
      to EXIT. Check if ZEROIZE(K_1) and ZEROIZE(K_2) on path.

Structurally identical to S4 but for decapsulation context.
"""
...
from hybridlint.checkers.s4_encap_zeroize import (
...
class S5DecapZeroizeChecker(BaseChecker):
    """S5-1 + S5-2: Undestroyed intermediate key material during decapsulation."""
    ...
    def check(self, cpg: FunctionCPG) -> list[Finding]:
        ...
        # Graph-based: need both classical and PQC decap nodes
        ...
        # Also try generic classical/PQC ops that look like decaps
        if not decap_1:
        ...
        # Fallback: if function name suggests decaps but no specific decap nodes,
        # use any classical + PQC op pair
    ...
    def _check_s5_2(self, cpg: FunctionCPG, cfg: nx.DiGraph,
        ...
        if between:
            findings.append(Finding(
                ...
                evidence=(
                    f"Intermediate key zeroized between DECAP_1 "
            ...
            findings.append(Finding(
                ...
                verdict=Verdict.UNCERTAIN, severity=Severity.MEDIUM,
                evidence=(
                    f"Intermediate key persists between decaps "
            ...
            findings.append(Finding(
                ...
                verdict=Verdict.FAIL, severity=Severity.HIGH,
                evidence=(
                    f"No zeroize of intermediate key between DECAP_1 "
                    f"(L{first_op.line}) and DECAP_2 (L{second_op.line}) "
                    f"or after."
                ),
