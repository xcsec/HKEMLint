"""Two-pass Cypher labeler for CPG nodes stored in Neo4j.

Pass 1 — SET op_label + component via keyword matching.
Pass 2 — SET val_label via op_label + component + variable-name heuristics.

Mirrors the logic in ``hybridlint.cpg.labeler`` but executes entirely
inside Neo4j so that the labeling is visible to subsequent Cypher
checker queries without a round-trip.
"""
...
# ═══════════════════════════════════════════════════════════════════════
# Keyword lists (same as hybridlint.locator.keywords, inlined for Cypher)
# ═══════════════════════════════════════════════════════════════════════
...
# ── Crypto-op keywords (fine-grained) ──────────────────────────────
...
# ═══════════════════════════════════════════════════════════════════════
# Helper: build a Cypher ANY() predicate for keyword containment
# ═══════════════════════════════════════════════════════════════════════

def _any_contains(prop: str, keywords: list[str]) -> str:
    """Return a Cypher expression: any(kw IN [...] WHERE n.<prop> CONTAINS kw)"""
...
def label_nodes_cypher(conn: Neo4jConnection) -> None:
    """Run the two-pass labeling on all :CPGNode in the database."""
    _pass1_op_labels(conn)
    _pass2_val_labels(conn)


# ═══════════════════════════════════════════════════════════════════════
# Pass 1 — op_label + component
# ═══════════════════════════════════════════════════════════════════════

def _pass1_op_labels(conn: Neo4jConnection) -> None:
    """Assign op_label and component to every :CPGNode.

    Order matters — first match wins (same priority as Python labeler).
    Each SET only touches nodes still labeled NONE to enforce priority.
    """
    ...
    # 2. ERROR_CHECK (if-statement with error pattern)
    ...
    # 3. ERROR_HANDLER (goto / error return)
    ...
    # 9. PARAM (hybrid group ID constants)
    stmts.append(f"""
        MATCH (n:CPGNode)
        WHERE n.op_label = 'NONE'
    ...
    # 13. KEYGEN — classical  (component = 1)
    stmts.append(f"""
        MATCH (n:CPGNode)
        WHERE n.op_label = 'NONE'
    ...
    # 14. ENCAP — classical  (component = 1)
    stmts.append(f"""
        MATCH (n:CPGNode)
        WHERE n.op_label = 'NONE'
    ...
    # 15. Generic PQC (best-effort op guess)
    stmts.append(f"""
        MATCH (n:CPGNode)
        WHERE n.op_label = 'NONE'
...
def _pass2_val_labels(conn: Neo4jConnection) -> None:
    ...
    # ── from variable-name heuristics (only for still-unlabeled) ───
    ...
    # ── propagate via dataflow from crypto ops ─────────────────────
